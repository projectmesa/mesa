    ...
    def update_strategies(self):
        ...
        for strategy in self.strategies:
            ...
            for _ in range(self.memory_size):
                prediction = self.predict_attendance(
                    strategy, self.model.history[week : week + self.memory_size]
                )
                score = score + abs(
                    self.model.history[week + self.memory_size] - prediction
                )
                week += 1
            if score <= best_score:
                best_score = score
                self.best_strategy = strategy
        if (self.model.history[-1] > self.crowd_threshold and self.attend) or (
            self.model.history[-1] < self.crowd_threshold and self.attend is False
        ):
            self.utility -= 1
        else:
            self.utility += 1
...
class BarCustomerIBLT(Agent):
    def __init__(self, unique_id, model, decay, crowd_threshold):
        ...
        self.agent = pyibl.Agent(
            "BarCustomer", ["Attendance"], decay=decay, noise=random.uniform(0.1, 1.5)
        )
