...
def test_agent_portrayal_style_update_non_existent_attribute():
    ...
    style = AgentPortrayalStyle()
    with pytest.raises(
        AttributeError, match=re.escape("'AgentPortrayalStyle' object has no attribute 'shape'")
    ):
...
def test_property_layer_style_post_init_neither_color_nor_colormap_error():
    ...
    with pytest.raises(ValueError, match=re.escape("Specify one of 'color' or 'colormap'")):
        PropertyLayerStyle()
