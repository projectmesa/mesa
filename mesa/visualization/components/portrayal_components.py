...
from typing import Optional, Union, ClassVar, Set, Dict
...
class AgentPortrayalStyle:
    ...
    # Default values
    color: Optional[Union[str, tuple]] = "tab:blue"
...
class PropertyLayerStyle:
    ...
    def __post_init__(self):
        ...
        if isinstance(self.colormap, str) and self.colormap is not None:
            if self.colormap not in plt.colormaps():
                raise ValueError(f"Invalid colormap name: {self.colormap}")
        
        # Validate numeric ranges
        if self.alpha is not None and (not isinstance(self.alpha, (int, float)) or not 0 <= self.alpha <= 1):
