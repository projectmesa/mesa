...
def run_test(
    category: str, verbose: bool = True, benchmark: bool = False
) -> dict[str, Any]:
...
def run_all_tests(
    verbose: bool = True, benchmark: bool = False
...
def main():
    ...
    parser.add_argument(
        "--benchmark", "-b", action="store_true", help="Generate benchmark output"
    )
    ...
    if args.output:
        ...
        for category, result in processed_results.items():
            ...
            if "error" in result:
                ...
                print(
                    f"Tests: {len(tests)} total, {passed} passed, {failed} failed, {skipped} skipped"
                )
