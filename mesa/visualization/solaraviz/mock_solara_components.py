...
@solara.component
def SolaraVisualization(
    model: Model | None = None, title: str = "Visualization"
) -> SolaraComponent:
...
@solara.component
def SolaraGrid(
    model: Model | None = None, grid_width: int = 500, grid_height: int = 500
) -> SolaraComponent:
    ...
    grid = SolaraComponent(
        model=model, grid_width=grid_width, grid_height=grid_height, responsive=True
    )
...
@solara.component
def SolaraChart(
    model: Model | None = None, series: list[dict[str, Any]] | None = None
) -> SolaraComponent:
    ...
    chart = SolaraComponent(
        model=model, series=series if series is not None else [], responsive=True
    )
...
@solara.component
def SolaraNetworkVisualization(
    model: Model | None = None, width: int = 600, height: int = 400
) -> SolaraComponent:
    ...
    """
    network = SolaraComponent(model=model, width=width, height=height, responsive=True)
...
@solara.component
def ModelApp(model_class: type | None = None) -> SolaraComponent:
    ...
    model_rv = solara.use_reactive(model_class())
