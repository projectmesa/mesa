...
def test_viz_component_interface_stability(example_model_name):
    ...
    try:
        ...
        for viz_name, viz_func in visualizations.items():
            ...
            assert "model" in sig.parameters, f"{viz_name} should accept a 'model' parameter"
            
            # Check that model parameter is optional
            assert sig.parameters["model"].default is None or sig.parameters["model"].default is inspect.Parameter.empty, \
                f"{viz_name}'s 'model' parameter should be optional or required"
    except (KeyError, AttributeError, ImportError) as e:
        pytest.skip(f"Could not check component interfaces for {example_model_name}: {e}")
...
def test_solara_component_existence(example_model_name):
    ...
    try:
        ...
        if not has_grid and not has_chart:
            pytest.xfail(f"Model {example_model_name} doesn't have common visualization types")
    except (KeyError, AttributeError) as e:
        pytest.skip(f"Could not check component existence for {example_model_name}: {e}")
