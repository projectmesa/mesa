...
def test_app_initialization_performance(example_model_name):
    ...
    try:
        ...
        assert initialization_time < 1.0, (
            f"App initialization took too long: {initialization_time:.2f}s"
        )
    except (KeyError, AttributeError, ImportError) as e:
        pytest.skip(
            f"Could not measure app initialization for {example_model_name}: {e}"
        )
...
def test_model_step_with_visualization_performance(example_model_name):
    ...
    try:
        ...
        overhead = (
            viz_step_time / base_step_time if base_step_time > 0 else float("inf")
        )
        if overhead > 2.0:
            # This is not a hard failure, just a warning
            pytest.xfail(
                f"Visualization overhead too high: {overhead:.2f}x for {example_model_name}"
            )
    except (KeyError, AttributeError, ImportError) as e:
