...
def SpaceDrawer(model):
    ...
    im = ax.imshow(np.ma.masked_where(model.grid.sugar.data<=1, model.grid.sugar.data), cmap="spring", origin="lower" )
    fig.colorbar(im, orientation="vertical")
    # Spice
    ax.imshow(np.ma.masked_where(model.grid.spice.data<=1, model.grid.spice.data), cmap="winter", origin="lower")
    # Trader
...
page = SolaraViz(
    model,
    components=[SpaceDrawer, make_plot_component("#Traders"), make_plot_component("Price")],
